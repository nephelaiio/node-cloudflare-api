...
const wait: (period: number) => Promise<void> =
  async (period) => new Promise((resolve) => {setTimeout(resolve, period)})
const retry: (fn: AsyncFn, times: number, delay: number) => Promise<any> =
  async (fn, times, delay) => {
    try {
      return await fn();
    } catch (e: any) {
      if (times <= 0) {
...
const timedFetch: (resource: string, options: any) => Promise<any> =
...
const api: (options: ApiOptions) => Promise<any> =
  async (options) => {
    const {
      token,
      path,
